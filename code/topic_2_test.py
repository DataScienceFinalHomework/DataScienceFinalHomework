...
def load_series_results_from_json(json_path):
    """
    从 JSON 文件加载绘图需要的数据结构：
    """
    with open(json_path, 'r', encoding='utf-8') as f:
        raw_data = json.load(f)

    series_results = {}
    for series_name, data in raw_data.items():
        entity_frequencies = data.get('entities', {})
        sorted_entities = sorted(entity_frequencies.items(), key=lambda item: item[1], reverse=True)
...
def plot_rank_frequency(series_results):
    """
    绘制每个 Series 的 rank vs 出现频率图
    """
    ...
    fig, axes = plt.subplots(num_series, 1, figsize=(12, 5*num_series))
    
    if num_series == 1:
        axes = [axes]
    
    for idx, (series_name, data) in enumerate(series_results.items()):
        if data is None:
            continue
        
        sorted_entities = data['entities']
        ranks = np.arange(1, len(sorted_entities) + 1)
        frequencies = [freq for _, freq in sorted_entities]
        
        ax = axes[idx]
        ax.scatter(ranks, frequencies, alpha=0.6, s=50, color='steelblue')
        ax.plot(ranks, frequencies, alpha=0.3, color='steelblue')
        
        ax.set_xlabel('Rank (按出现频率排序)', fontsize=12)
        ax.set_ylabel('出现频率 (次数)', fontsize=12)
        ax.set_title(f'Series: {series_name} - Rank vs 出现频率', fontsize=14, fontweight='bold')
        ...
        # 在log-log坐标上叠加回归线
        regression_stats = data.get('regression_stats')
        if regression_stats and len(ranks) >= 2:
            slope = regression_stats['slope']
            intercept = regression_stats['intercept']
            fit_freq = np.exp(intercept) * (ranks ** slope)
            ax.plot(ranks, fit_freq, color='tomato', linewidth=2, label='log-log回归线')
            ax.legend()

            text_lines = [
                f"r = {regression_stats['correlation']:.4f}",
                f"R^2 = {regression_stats['r_squared']:.4f}",
                f"线性关系: {'是' if regression_stats['is_approximately_linear'] else '否'}",
            ]
            ax.text(
                0.05,
                0.95,
...
def plot_rank_frequency_from_json(json_path):
    """
    单独读取 JSON 结果并绘图
    """
    series_results = load_series_results_from_json(json_path)
    if not series_results:
        print("\n⚠ JSON 中没有可绘制的数据，已跳过绘图。")
        return

    print("\n正在基于 JSON 结果绘制图表...")
    plot_rank_frequency(series_results)
    
    plt.tight_layout()
    
    # 保存图表
    output_path = os.path.join(SCRIPT_DIR, "../entity_analysis_rank_frequency.png")
...
async def main():
    ...
    # 绘图部分与 API 调用分离：这里只基于 JSON 再绘图
    if series_results:
        plot_rank_frequency_from_json(output_json)
    else:
        print("\n⚠ 实体结果为空：仅保存了空 JSON（{}），未生成图表。")
